...
**Status**: D0 + D1 + D2 + D3 + D4 shipped. D5 remains explicitly optional.
...
| D3 | **Shipped** | Wired `type: technical-document` through the generic Document Model pipeline, including native Typst and EPUB technical-document renderers, technical-document asset resolution, persistent generated asset staging, and regression coverage. `type: book` remains on the existing path. | D2 |
| D4 | **Shipped** | Validated the pipeline against `RideTogether EngineeringDesign.md` and the structurally different `APIEngineeringReference.md`, covering external images, deeper headings, ordered/unordered lists, code blocks, JSON, tables, PDF, and EPUB generation. Validation exposed the current Markdown-table rendering limitation; this is a follow-up capability, not a D4 blocker. Full suite: 207 passed. | D3 |
