...
from digitization.combined import ProfileRoutingOCR, SourceSegment, combine_pdfs, remap_result_pages, split_page_durations
...
def test_profile_routing_ocr_routes_pages_in_order(tmp_path):
    ...
    assert [router.ocr_image(tmp_path / f"p{i}.png") for i in range(1, 5)] == [
...
def test_remap_result_pages_restores_original_provenance():
    ...
    pages = tuple(
        PageOCR(Path("combined.pdf"), number, Path(f"p{number}.png"), f"page {number}")
        for number in range(1, 5)
    )
    result = DigitizationResult(Path("combined.pdf"), pages)

    remapped = remap_result_pages(result, segments)
...
def test_split_page_durations_preserves_source_boundaries():
    ...
    segments = (
        SourceSegment(first, "prose", 1, 2),
        SourceSegment(second, "code", 3, 5),
    )
...
def test_combine_pdfs_creates_one_pdf_and_page_map(tmp_path):
    ...
    combined, segments = combine_pdfs(
        [(first, "prose"), (second, "code")],
        tmp_path / "combined.pdf",
    )

    document = fitz.open(combined)
    try:
